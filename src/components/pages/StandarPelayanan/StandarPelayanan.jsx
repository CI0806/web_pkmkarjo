...
const BASE_URL = "https://admin.pkmkarjotarakan.com/uploads/standar_pelayanan/";
...
const StandarPelayanan = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await services.standarPelayanan.getStandarPelayanan();
        if (response.status === "success") setData(response.data);
      } catch (error) {
        console.error("Gagal mengambil data standar pelayanan:", error);
      } finally {
        setLoading(false);
      }
    };
                ...
                : (
                  <Grid item size={{ xs: 12 }}>
                    <Box sx={{ textAlign: "center", py: 10 }}>
                      <Typography color="text.secondary">Belum ada data Standar Pelayanan.</Typography>
                    </Box>
                  </Grid>
                )}
            </Grid>
